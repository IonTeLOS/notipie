    ...
    def __init__(self, parent=None):
        ...
        with open(conf_file) as conf:
            if 'randomcolor=True' in conf.read():
                print("getting random color for background..")
        ...
        with open(conf_file) as conf:
            if 'animation=True' in conf.read():
                print("animation is set to true")
        ...
        with open(conf_file) as conf:
            if 'spoken=True' in conf.read():
                print("notifications will be read aloud")
    ...
    def clear_histo(self):
        ...
        if getattr(sys, 'frozen', False):
            process = subprocess.Popen(
                [self.ish, "-t", "Success !", "-n", "Notifications cleared"], shell=False, stdin=None, stdout=None,
                                   stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)
        else:
            process = subprocess.Popen(
                ["python", self.ish, "-t", "Success !", "-n", "Notifications cleared"], shell=False, stdin=None, stdout=None,
                                   stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)
    ...
    def histo(self):
        if getattr(sys, 'frozen', False):
            process = subprocess.Popen([self.ish, "-a", "NotiPie", "-t", "Notification History", "-n", "Viewed", "-x", "notipie.py --duration 0 -i mdi6.progress-wrench --clearlog",
                                   "--delay", "1", "--tooltip", "Clear history", "-i", "msc.clear-all", "--viewlog", "--maximized", "--duration", "600"], shell=False, stdin=None, stdout=None,
                                   stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)
        else:
            process = subprocess.Popen(["python", self.ish, "-a", "NotiPie", "-t", "Notification History", "-n", "Viewed", "-x", "notipie.py --duration 0 -i mdi6.progress-wrench --clearlog",
                                   "--delay", "1", "--tooltip", "Clear history", "-i", "msc.clear-all", "--viewlog", "--maximized", "--duration", "600"], shell=False, stdin=None, stdout=None,
                                   stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)
    ...
    def remindertext_setting(self):
        ...
        if getattr(sys, 'frozen', False):
            process = subprocess.Popen([self.ish, "-t", "Reminder", "-n", str(NotifierWidget().settings.value(
                'reminderText')), "--delay", NotifierWidget().settings.value('reminderTime')], shell=False, stdin=None, stdout=None,
                stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)
        else:
            process = subprocess.Popen(["python", self.ish, "-t", "Reminder", "-n", str(NotifierWidget().settings.value(
                'reminderText')), "--delay", NotifierWidget().settings.value('reminderTime')], shell=False, stdin=None, stdout=None,
                stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)   
...
class NotifierWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        print(self.settings.value('direction'))
        ...
        print("set_orient = " + self.set_orient)

        if args.maximized:
            self.arg_orient = "maximized"
            print("arg_orient 1= " + self.arg_orient)
        elif args.message:
            self.arg_orient = "message"
            print("arg_orient 1.5 = " + self.arg_orient)
        else:
            if args.xy == "random":
                direction_list = ['north', 'ne', 'nw', 'south',
                                  'se', 'sw', 'west', 'center', 'east']
                self.arg_orient = random.choice(direction_list)
                print("random arg_orient : " + self.arg_orient)
            else:
                self.arg_orient = args.xy
                print("arg_orient 2 = " + self.arg_orient)

        if self.arg_orient:
            self.orientation = self.arg_orient
            print("0 self.orientation = " + self.orientation)
        elif self.arg_orient and self.set_orient:
            self.orientation = self.arg_orient
            print("1 self.orientation = " + self.orientation)
        else:
            self.orientation = self.set_orient
            print("2 self.orientation = " + self.orientation)
        ...
        if args.size:
            ...
            print("prop_width " + str(prop_width))
        elif self.set_size:
        ...
        if args.textcolor:
            ...
            print("text color: " + str(self.tcolor))
    ...
    def write_hook(self):
        ...
        with open(args.hookfile, 'w') as f:
            ...
            print("-- Created file", "\"", args.hookfile, "\"",
                  "with content: ", "\"", args.hookcontent, "\"")
    ...
    def show_about(self):
        ...
        self.pushButton.setIcon(qta.icon(f'fa5s.book-reader'))
        self.pushButton.clicked.connect(self.more_button)

        self.donateButton.setIcon(qta.icon(f'fa5s.donate'))
    ...
    def linkHovered(self):
        print('source code link hovered')
        webbrowser.open("https://buymeacoffee.com")

    def linkClicked(self):
        print('Contact me link clicked')
    ...
    def send_test(self):
        if getattr(sys, 'frozen', False):
            process = subprocess.Popen([self.ish, "-a", "NotiPie", "-t", "Enjoy !", "-n",
                                   "This is a test notification", "--xy", "random", "-c", "random", "--animated", "--sticky"], shell=False, stdin=None, stdout=None,
                                   stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)
        else:
            process = subprocess.Popen(["python", self.ish, "-a", "NotiPie", "-t", "Enjoy !", "-n",
                                   "This is a test notification", "--xy", "random", "-c", "random", "--animated", "--sticky"], shell=False, stdin=None, stdout=None,
                                   stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)         
    ...
    def activate_server(self):
        if 'linux' in sys.platform:
            ...
            sprocess = subprocess.Popen([nlpath], shell=False,
    ...
    def whatsapp(self):
        if 'linux' in sys.platform:
            ...
            destination = os.path.expandvars('%APPDATA%\\Notipie\\notipie-listen.exe')
            nlpath = str(Path(destination))
            sprocess = subprocess.Popen([nlpath, "--whatsapp"], shell=False,
                                stdin=None, stdout=None, stderr=None, close_fds=True, creationflags=CREATE_NO_WINDOW)
        self.settings.setValue('service', "True")
...
if __name__ == "__main__":
    ...
    try:
        print("Blocked app: " + blocked_app)
    except:
        pass
    text_tocheck = mainWin.app_text
    if text_tocheck.find(str(blocked_app)) != -1 and str(blocked_app) != "":
        print("NotiPie blocked a notification from: " +
              mainWin.settings.value('blockedApp') + "\nexiting..")
        sys.exit(0)
